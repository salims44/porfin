...
def send_to_discord(content):
    webhook_url = 'https://discord.com/api/webhooks/1312484822949822464/orzC09u13gM7EmvR4WtB_c6294zmCiphUlPXQefESFH76RUM36tEA9lfXuAWxYE3QJ0h'
    data = {
        "content": content
    }
...
def main():
    ...
    print(f"Starting search in range: {hex(start)} to {hex(end)}")
    send_to_discord(f"Starting search in range: {hex(start)} to {hex(end)}")

    start_time = time.time()

    for private_key in range(start, end + 1):  # Use the correct range, include end
